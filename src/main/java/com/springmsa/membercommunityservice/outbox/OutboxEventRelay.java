package com.springmsa.membercommunityservice.outbox;
    ...
    public void publishPending() {
        ...
        for (PendingEvent event : events) {
            try {
                ...
                jdbcTemplate.update(
                        "UPDATE community_service.outbox_events SET published_at = ?, attempts = attempts + 1, last_error = NULL WHERE event_id = ?",
                        OffsetDateTime.now(ZoneOffset.UTC), event.eventId());
            } catch (Exception exception) {
                String message = exception.getMessage() == null ? exception.getClass().getSimpleName() : exception.getMessage();
                jdbcTemplate.update(
                        "UPDATE community_service.outbox_events SET attempts = attempts + 1, last_error = ? WHERE event_id = ?",
